...
class CarAgencyManager {
  ...
  #getAgency(idOrName, id = 0) { // search using id and name or using id only.
    for (const agency of this.#agencies) {
      if (agency.agencyId === id || agency.agencyId === idOrName || agency.agencyName === idOrName) {
        return agency;
      }
    }
    return null;
  ...
  #findCarIndexAndModel(carId, agency) {
    for (const car of agency.cars) {
      if (car.models) {
        const carIndex = car.models.findIndex((item) => item.carNumber === carId);
        if (carIndex !== -1) {
          return {
            models: car.models,
            index: carIndex
  ...
  searchAgency(idOrName) {
    this.#isNull(idOrName);
    const agency = this.#getAgency(idOrName);
    if (agency) {
      return { ...agency }; //------ need fix to {...agency}
    }
    return null;
  }
  ...
  addCarToAgency(agencyId, car) {
    this.#isNull(agencyId, car);
    const agency = this.#getAgency(0, agencyId);
    if (!agency) {
      return false;
    }
    for (const item of agency.cars) {
      if (item.brand === car.brand) {
        for (const specificCar of car.models) {
          item.models.push(specificCar);
          return true;
        }
      }
    }
  ...
  removeCarFromAgency(agencyId, carId) {
    this.#isNull(agencyId, carId);
    const agency = this.#getAgency(0, agencyId);
    const car = this.#findCarIndexAndModel(carId, agency);
    console.log(car);
    if (car) {
      car.models.splice(car.carIndex, 1);
      return true;
    }
    return false;
  }
  ...
  changeAgencyCashOrCredit(agencyId, cashOrCredit) {
    this.#isNull(agencyId, cashOrCredit);
    this.#isNumber(cashOrCredit);
    const agency = this.#getAgency(0, agencyId);
    if (agency.cash + agency.credit < cashOrCredit) {
      return false;
    }
    agency.cash = agency.cash - cashOrCredit;
    if (agency.cash < 0) {
      agency.credit = agency.credit - (agency.cash * -1);
      agency.cash = 0;
    }
    return true;
  }
...
class CustomerManager {
  ...
  #getCustomer(idOrName, id = 0) {
    // find using id only or id and name.
    for (const customer of this.#customers) {
      if (customer.id === id || customer.id === idOrName || customer.name === idOrName) {
        return customer;
      }
    }
    return null;
  ...
  // --- return: the copy customer.
  searchCustomer(idOrName) { //---- I need handle if there are more one customer whit the same name?--- 
    this.#isNull(idOrName);
    const customer = this.#getCustomer(idOrName);
    if (customer) {
      return { ...customer };
    }
    return null;
  ...
  changeCustomerCash(customerId, cash) {
    this.#isNull(cash);
    const customer = this.#getCustomer(0, customerId);
    if (customer) {
      customer.cash = cash;
      return true;
    }
  ...
  getCustomerTotalCarValue(customerId) {
    this.#isNull(customerId);
    const customer = this.#getCustomer(0, customerId);
    if (customer) {
      return customer.cars.reduce((acc, current) => acc + current.price, 0);
    }
    return null;
  }
}
...
console.log('Change Agency Cash: ', carAgencyManager.changeAgencyCashOrCredit('Plyq5M5AZ', 200000));
